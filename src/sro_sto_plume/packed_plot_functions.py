...
def plot_rsm_figure(plotter, fig, axes, files, sample_names, cbar_ax, peak_z_range=None, draw_peak=True, draw_peak_line=True, draw_ideal_Qz_line=False, ideal_Qz=4.79, i_start=0, text_locs=None, label=True):
    ...
    # Draw RSMs
        if i == 0:
            Qx, Qz, intensity = plotter.plot(file, ax, figsize=None, cbar_ax=cbar_ax, ignore_yaxis=False)
        else:
    ...
    if text_locs:
        for i, (ax, sample_name, color, text_loc) in enumerate(zip(axes, sample_names, colors, text_locs)):
            marker_line_label = Line2D([0], [0], color=color, linestyle='-', marker='o', markersize=6)
            ax.legend(handles=[marker_line_label], loc='lower right', frameon=False, bbox_to_anchor=(1.15, 0.04), fontsize=8)
            add_text_to_figure(fig, sample_name, text_loc, fontsize=8, color='white', ha='center')

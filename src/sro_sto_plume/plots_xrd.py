...
def plot_xrd_multiple(
    xrd_files, rocking_curve_files, rsm103_files, label=True,
    rsm_peak_labels=('SrTiO₃\n103', 'SrRuO₃\n103'),
    rsm_peak_label_offsets=((7, 0), (5, 0)),
    rsm_peak_label_kwargs=None,
):
    """
    Compose a 3-panel figure:
      [2_1] XRD (2θ–ω) traces
      [2_2] Rocking curves
      [3]   RSM(103) strip, with optional substrate/film peak identities

    ``label`` controls the a-h panel letters only. The RSM peak identities,
    offsets, and text style are configured here so every Figure 2 variant
    generated by this function stays synchronized.
    """
    ...
    if rsm_peak_label_kwargs is None:
        rsm_peak_label_kwargs = {
            'ha': 'left',
            'va': 'center',
    ...
    plot_rsm_figure(
        ...
        peak_z_range_film=(4.765, 4.800),
        plane="103",
        ideal_q=ideal_q,
        peak_labels=rsm_peak_labels,
        peak_label_offsets=rsm_peak_label_offsets,
        peak_label_kwargs=rsm_peak_label_kwargs,
        label=label,

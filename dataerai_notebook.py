"""Continuous Dataerai provenance capture for the SrRuO3 tutorial notebooks.
...
* assigned arrays, tables, figures, and configuration dictionaries; and
...
SCHEMA = "org.dataerai.srruo3-notebook-provenance/v1"
COURSE = {
    "name": "SrRuO3 Plume Dynamics",
    "repository": "https://github.com/m3-learning/SrRuO3_Plume_Dynamics",
    "instrument": "PLD, plume imaging, AFM, XRD, confocal microscopy",
}
...
_MANAGED_PROJECT_NAME = "SrRuO3 Plume Dynamics notebook records"
_FIGURE_FORMATS = {"png", "jpg", "jpeg", "svg"}
_TRANSIENT_DATAERAI_ERROR = re.compile(
    r"HTTP (?:429|502|503|504)\b|"
    r"timed out|did not complete within|connection (?:was )?closed",
    re.IGNORECASE,
)
_DEFAULT_TRANSFER_TIMEOUT_SECONDS = 3600.0
...
def _json_safe(value: Any, *, key: str | None = None, depth: int = 0) -> Any:
    ...
    if isinstance(value, (list, tuple, set, frozenset)):
        sequence = list(value)
        result = [_json_safe(item, depth=depth + 1) for item in sequence]
        return result
...
class NotebookProvenance:
    ...
    def _tutorial_metadata(self) -> dict[str, Any]:
        return {
            "notebook_path": self.notebook_relative_path,
            "notebook_name": self.notebook_path.stem,
            "track": "analysis",
            "topic": self.notebook_path.stem.replace("_", " "),
        }
    ...
    def _environment_metadata(self) -> dict[str, Any]:
        ...
        return {
            ...
            "packages": {
                ...
                "Plume-Learn": _package_version("Plume-Learn"),
                "AFM-Learn": _package_version("AFM-Learn"),
                "XRD-Learn": _package_version("XRD-Learn"),
                "M3Learning-Util": _package_version("M3Learning-Util"),
            },
            "git": {
    ...
    def _tags(self, record_kind: str, *extra: str) -> list[str]:
        return [
            "data-notebook",
            "srruo3-plume-dynamics",
            "pulsed-laser-deposition",
            "jupyter",
            "continuous-provenance",
            f"srruo3-run:{self.run_id}",
    ...
    def _managed_project_id(self, user_email: str) -> str:
        ...
        project = create_project(
            ...
            description=(
                "Automatically created by the SrRuO3 Plume Dynamics notebooks so "
    ...
    def start(self) -> "NotebookProvenance":
        ...
        if not getattr(self, "_client_connected", False):
            self.client.connect()
        ...
        self.run_asset_id = self._upload(
            run_manifest,
            title=_title(
                "SrRuO3 notebook run", self.notebook_relative_path, self.run_id
            ),
            description=(
    ...
    def _capture_dependencies(self) -> None:
        ...
        self.dependency_manifest_asset_id = self._upload(
            manifest_path,
            title=_title(
                "SrRuO3 dependency manifest", self.notebook_relative_path, self.run_id
            ),
            description=(
        ...
        for path, entry in zip(dependencies, manifest_entries):
            ...
            dependency_asset_id = self._upload(
                path,
                title=_title(
                    "SrRuO3 notebook dependency",
                    entry["path"],
                    f"sha256:{entry['sha256'][:12]}",
                ),
                description=(
                    f"Content-addressed {role.replace('_', ' ')} used by SrRuO3 "
                    "plume-dynamics notebook runs."
                ),
                metadata=metadata,
                tags=self._origin_tags(
                    "notebook_dependency",
    ...
    def _record_type_for_upload(
        ...
        is_simulation = "/simulation/" in f"/{self.notebook_relative_path}"
        run_type = "simulation" if is_simulation else "analysis"

        if record_kind == "notebook_source":
        ...
        for display_index, display in enumerate(displays, start=1):
            ...
            for mime, value in display.data.items():
                if mime in {"image/png", "image/jpeg", "image/svg+xml"}:
                    ...
                    display_record["mime"][mime] = value
    ...
    def _serialize_assigned_value(
        ...
        try:
            import numpy as np
        except ImportError:  # pragma: no cover - analysis notebooks require numpy
            np = None
        ...
        if np is not None and isinstance(value, (Mapping, list, tuple)):
            arrays = {}
            seen = set()

            def encode(item, key=None):
                if key and _SECRET_KEY.search(str(key)):
                    return "<redacted>"
                if isinstance(item, np.ndarray):
                    label = f"array_{len(arrays):04d}"
                    arrays[label] = item
                    return {"array": label, "shape": list(item.shape), "dtype": str(item.dtype)}
                if isinstance(item, (Mapping, list, tuple)):
                    ...
                    try:
                        if isinstance(item, Mapping):
                            return {str(k): encode(v, k) for k, v in item.items()}
                        return {"container": type(item).__name__, "items": [encode(v) for v in item]}
                    finally:
                        seen.remove(id(item))
            ...
            if arrays:
                path = cell_dir / f"{safe_name}.npz"
                np.savez_compressed(path, **arrays, __structure__=np.array(json.dumps(structure)))
                return PendingArtifact(path=path, name=name, role=role, file_format="npz",
                    python_type=python_type, details={"array_count": len(arrays),
    ...
    def finish(self) -> str | None:
        ...
        status = "completed_with_capture_errors" if self.errors else (
            "completed_with_execution_errors" if self.execution_failures else "completed"
        )
        summary = {
        ...
        try:
            self.summary_asset_id = self._upload(
                summary_path,
                title=_title(
                    "SrRuO3 notebook run summary",
                    self.notebook_relative_path,
                    self.run_id,
                ),
                description=(
                    "Terminal summary for a continuous SrRuO3 notebook provenance run."
                ),
                metadata=summary,
                tags=self._tags(
